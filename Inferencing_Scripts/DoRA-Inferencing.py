...
def load_video_sequence(video_path, frame_seconds_list, input_size=448, max_num=1):
    """
...
def generate_text_from_sample(model, tokenizer, sample):
    ...
    pixel_values, num_patches_list = load_video_sequence(sample[0], frame_seconds_list=[1, 10, 20, 30], max_num=1)
    if use_cuda:
...
def run_single_test(model, tokenizer, dataset, max_samples=None):
    ...
    os.makedirs(output_dir, exist_ok=True)
    ref_txt_path = os.path.join(output_dir, "dora_os_references.txt")
    hyp_txt_path = os.path.join(output_dir, "dora_os_predictions.txt")
    combined_txt_path = os.path.join(output_dir, "dora_os_combined.txt")
...
def main():
    ...
    print(f"Loading DoRA weights from: {checkpoint_path}")
    lm = model.language_model
    lm = PeftModel.from_pretrained(lm, checkpoint_path)
    ...
    output_directory = "Your_path"  # Updated to reflect DoRA model
    ...
    save_results_to_file(result, f"internVL2B_dora_results_{timestamp}.json", output_directory)
        
    
    # Save all results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    # Also save a wrapper object for compatibility
    save_results_to_file([result], f"internVL2B_dora_all_results_{timestamp}.json", output_directory)
