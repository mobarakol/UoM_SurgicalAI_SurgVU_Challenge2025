...
def run_single_test(model, tokenizer, dataset, max_samples=None):
    ...
    os.makedirs(output_dir, exist_ok=True)
    ref_txt_path = os.path.join(output_dir, "internVL3-2B_references.txt")
    hyp_txt_path = os.path.join(output_dir, "internVL3-2B_predictions.txt")
    combined_txt_path = os.path.join(output_dir, "internVL3-2B_combined.txt")
...
def main():
    ...
    save_results_to_file(result, f"internVL3-2B_results_{timestamp}.json", output_directory)
        
    
    # Save all results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    # Also save a wrapper object for compatibility
    save_results_to_file([result], f"internVL3-2B_all_results_{timestamp}.json", output_directory)
