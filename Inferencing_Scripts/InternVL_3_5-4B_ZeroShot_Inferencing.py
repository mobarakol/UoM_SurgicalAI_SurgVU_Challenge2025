...
HUGGINGFACE_TOKEN = "Yourtoken_here"  # Update with your actual token
...
def generate_text_from_sample(model, tokenizer, sample):
    ...
    pixel_values, num_patches_list = load_video(sample[0], num_segments=8, max_num=1)
    if use_cuda:
...
def run_single_test(model, tokenizer, dataset, max_samples=None):
    ...
    os.makedirs(output_dir, exist_ok=True)
    ref_txt_path = os.path.join(output_dir, "internVL3_5-4B_references.txt")
    hyp_txt_path = os.path.join(output_dir, "internVL3_5-4B_predictions.txt")
    combined_txt_path = os.path.join(output_dir, "internVL3_5-4B_combined.txt")
...
def main():
    ...
    root_dir = "Your_path_here"  # Update with your actual path
    ...
    output_directory = "Your_path_here"  # Specify output directory
    ...
    print("\n" + "=" * 60)
    print("Starting test (no system message)")
    print("=" * 60)
    try:
    ...
    save_results_to_file(result, f"internVL3_5-4B_results_{timestamp}.json", output_directory)
        
    
    # Save all results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    # Also save a wrapper object for compatibility
    save_results_to_file([result], f"internVL3_5-4B_all_results_{timestamp}.json", output_directory)
