...
print("=+= Loading InternVL3_5-4B-Instruct model from local path...")
try:
    # The path should correspond to where the model is copied in the Dockerfile.
    local_model_path = RESOURCE_PATH / "InternVL3_5-4B-Instruct"
...
def interf0_handler():
    ...
    pixel_values, num_patches_list = load_video(str(video_path), num_segments=16, max_num=1)
    if use_cuda:
